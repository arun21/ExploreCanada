...
register_activation_hook( __FILE__, 'run_this_function_when_plugin_activate' );
function run_this_function_when_plugin_activate() {
...
register_deactivation_hook( __FILE__, 'run_this_function_when_plugin_deactivate' );
function run_this_function_when_plugin_deactivate(){
...
function submit_user_query_handle(){
	if(isset($_POST['action']) && $_POST['formsdata']) {
		...
		if($action == "submit_user_query") {
			$name = $cfw_data['name'];
			$email = $cfw_data['email'];
			$subject = $cfw_data['subject'];
			$message = $cfw_data['message'];
			...
			if($wpdb->insert( $cfw_table_name, $cfw_columns_data, $cfw_data_format)) {
				...
				if($qfm == "") echo "Sorry! contact from not working properly. Please directly contact to site admin using this email: ".get_option( 'admin_email' ); else echo $qsm;
			}
		}
	}// end of isset
}
...
class form_Widget extends WP_Widget {
	...
	if(isset($all_setttings)){
		...
		<form id="user-contact-form" name="user-contact-form" class="cfw-form">
			<h2 class="text-center cwf-title"><?php echo $title_field; ?></h2>
			<p class="text-center cwf-desc"><?php echo $description_field; ?></p>

			<div class="form-group">
				<label for="name"><i class="fa fa-user" aria-hidden="true"></i> Name</label>
				<input type="text" class="form-control" id="name" name="name" value="" placeholder="<?php echo $name_field; ?>">
				<span class="cfw-error name-error alert alert-warning"><strong><?php echo $name_error_field; ?></strong></span>
			</div>

			<div class="form-group">
				<label for="email"><i class="fa fa-envelope" aria-hidden="true"></i> Email</label>
				<input type="text" class="form-control" id="email" name="email" value="" placeholder="<?php echo $email_field; ?>">
				<span class="cfw-error email-error alert alert-warning"><strong><?php echo $email_error_field; ?></strong></span>
				<span class="cfw-error email-error-2 alert alert-warning"><strong><?php echo $email_error_field_2; ?></strong></span>
			</div>

			<div class="form-group">
				<label for="subject"><i class="fa fa-asterisk" aria-hidden="true"></i> Subject</label>
				<input type="text" class="form-control" id="subject" name="subject" value="" placeholder="<?php echo $subject_field; ?>">
				<span class="cfw-error subject-error alert alert-warning"><strong><?php echo $subject_error_field; ?></strong></span>
			</div>

			<div class="form-group">
				<label for="message"><i class="fa fa-comment" aria-hidden="true"></i> Message</label>
				<textarea class="form-control" id="message" name="message" placeholder="<?php echo $message_field; ?>"></textarea>
				<span class="cfw-error message-error alert alert-warning"><strong><?php echo $message_error_field; ?></strong></span>
			</div>

			<div class="form-group">
				<button type="button" class="btn <?php echo $sb_button_color; ?>"  onclick="return ValidateForm();"><?php echo ucwords($sb_button_text); ?></button>
			</div>
		</form>
